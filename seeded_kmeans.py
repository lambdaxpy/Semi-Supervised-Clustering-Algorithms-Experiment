...
class SeededKMeans:
	def __init__(self, labeled_data: np.ndarray, unlabeled_data: np.ndarray, n_clusters: int=2,
		...
		self._unlabeled_data = np.array([np.append(data_point, -1) for data_point in unlabeled_data])
	...
	def _assign_clusters(self) -> None:
		...
		for i, data in enumerate(self._labeled_data):
			distances = [np.linalg.norm(np.subtract(data[:self._dimension - 1], centroid)) for centroid in
						 self._centroids]
			min_distance = min(distances)
			min_distance_cluster = distances.index(min_distance)
			self._labeled_data[i][self._dimension - 1] = min_distance_cluster
		# print(self._unlabeled_data)
...
if __name__ == "__main__":
	...
	kmeans = SeededKMeans(labeled_data, unlabeled_data, n_clusters=2)
	kmeans.cluster_data()
